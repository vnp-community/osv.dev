...
function AssetDetailSkeleton() {
  return (
    <div className="flex flex-col flex-1 overflow-hidden" style={{ background: "var(--color-bg-page, #0B1020)" }}>
      <div className="px-6 py-4 animate-pulse" style={{ background: "var(--color-bg-sidebar, #0F1629)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div className="col-span-2 flex flex-col gap-5">
          <div className="grid grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="rounded-2xl p-4 h-24" style={{ background: "var(--color-bg-card, #151B2F)" }} />
            ))}
          </div>
...
function AssetDetailContent({ asset, onBack }: { asset: any; onBack: () => void }) {
  ...
  const kpis = [
    { label: "Risk Score", value: String(asset.riskScore ?? "—"), color: "var(--color-status-error, #EF4444)" },
    { label: "Active Findings", value: String(asset.findingCount ?? 0), color: "var(--color-severity-high, #F97316)" },
    { label: "Open Ports", value: String(asset.openPorts ?? 0), color: "var(--color-primary, #4F8CFF)" },
    { label: "Last Scan", value: asset.lastScanAt ?? "—", color: "var(--color-status-success, #10B981)" },
  ];
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h2 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 16, fontWeight: 700, fontFamily: "monospace" }}>{asset.ip ?? asset.hostname}</h2>
              <span style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 14 }}>{asset.hostname}</span>
              <span className="px-2 py-0.5 rounded" style={{ background: "rgba(239,68,68,0.1)", color: "var(--color-status-error, #EF4444)", fontSize: 12 }}>{asset.environment ?? "Production"}</span>
            </div>
            <p style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 12, marginTop: 2 }}>{asset.os ?? "—"} · Last scanned {asset.lastScanAt ?? "—"}</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-3 gap-5">
          ...
          <div className="col-span-2 flex flex-col gap-5">
            ...
            {services.length > 0 && (
              <div className="rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div style={{ color: "var(--color-text-secondary, #9CA3AF)", fontSize: 12, fontWeight: 600, marginBottom: 12 }}>SERVICES & TECHNOLOGIES</div>
                  ...
                  {services.map((s) => (
                    <div key={s.name} className="flex items-center gap-4 rounded-xl p-3" style={{ background: "rgba(255,255,255,0.03)" }}>
                      ...
                      <div className="flex-1">
                        <div style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 13, fontWeight: 500 }}>{s.name}</div>
                        <div style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 11 }}>{s.version}{s.port ? ` · Port ${s.port}` : ""}</div>
                      </div>
                      {s.cves > 0 ? (
                        <span className="px-2 py-0.5 rounded" style={{ background: "rgba(239,68,68,0.1)", color: "var(--color-status-error, #EF4444)", fontSize: 11 }}>{s.cves} CVEs</span>
                      ) : (
            ...
            {/* Risk trend */}
            <div className="rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div style={{ color: "var(--color-text-secondary, #9CA3AF)", fontSize: 12, fontWeight: 600, marginBottom: 12 }}>RISK SCORE TREND</div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-2xl p-4" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div style={{ color: "var(--color-text-secondary, #9CA3AF)", fontSize: 11, fontWeight: 600, marginBottom: 10 }}>SYSTEM INFORMATION</div>
              {systemInfo.map(({ label, value }) => (
                <div key={label} className="flex justify-between py-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                  ...
                  <span style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 12 }}>{value}</span>
                </div>
              ))}
            </div>
            ...
            {scanHistory.length > 0 && (
              <div className="rounded-2xl p-4" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div style={{ color: "var(--color-text-secondary, #9CA3AF)", fontSize: 11, fontWeight: 600, marginBottom: 10 }}>SCAN HISTORY</div>
                {scanHistory.map((s) => (
                  <div key={s.scan} className="py-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    <div className="flex items-center justify-between">
                      <span style={{ color: "var(--color-primary, #4F8CFF)", fontSize: 12 }}>{s.scan}</span>
                      <span style={{ color: s.findings > 0 ? "#F59E0B" : "#10B981", fontSize: 11 }}>{s.findings} findings</span>
                    </div>
                    <div style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 11 }}>{s.date} · {s.type}</div>
                  </div>
                ))}
              </div>
            )}
