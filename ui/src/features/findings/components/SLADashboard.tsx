...
// Normalised shape the component will always work with
...
function normalise(raw: SLAOverviewRaw): SLANormalised {
  // Prefer real sla-service shape, fall back to gateway stub shape
  ...
  // Build upcoming breaches from at-risk findings (sorted by hours remaining)
  ...
  // Average days left is computed from at_risk findings hours_remaining
...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload?.length) return (
    <div className="rounded-xl p-2" style={{ background: "#1E2A45", border: "1px solid rgba(255,255,255,0.1)", fontSize: 11 }}>
      <div style={{ color: "var(--color-text-secondary, #9CA3AF)", marginBottom: 3 }}>{label}</div>
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-5 animate-pulse" style={{ background: "var(--color-bg-page, #0B1020)" }}>
      ...
      <div className="grid grid-cols-2 gap-4 mb-5">
        <div className="rounded-2xl h-48" style={{ background: "var(--color-bg-card, #151B2F)" }} />
        <div className="rounded-2xl h-48" style={{ background: "var(--color-bg-card, #151B2F)" }} />
      </div>
    </div>
  );
      ...
      {(sla) => {
        ...
        return (
          <div className="flex-1 overflow-y-auto px-6 py-5" style={{ background: "var(--color-bg-page, #0B1020)" }}>
            <div className="mb-5">
              <h2 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 18, fontWeight: 700 }}>SLA Dashboard</h2>
              <p style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 12 }}>Service Level Agreement tracking across all products</p>
            </div>

            {/* KPIs */}
            <div className="grid grid-cols-4 gap-4 mb-5">
              {[
                { label: "Overall Compliance", value: `${compliancePct}%`, trend: null, color: "var(--color-status-success, #10B981)", icon: CheckCircle },
                { label: "Compliant Findings", value: sla.compliant.toLocaleString(), sub: `of ${total} total`, color: "var(--color-status-success, #10B981)", icon: CheckCircle },
                { label: "SLA Breached", value: sla.breached, sub: `${sla.atRisk} at risk`, color: "var(--color-status-error, #EF4444)", icon: AlertTriangle },
                { label: "Avg Days Left", value: sla.avgDaysLeft, sub: "To SLA deadline", color: "var(--color-status-warning, #F59E0B)", icon: Clock },
                ...
                <div key={label} className="rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {trend && (
                      <span className="flex items-center gap-1 text-xs" style={{ color: "var(--color-status-success, #10B981)" }}>
                        <TrendingDown size={11} />{trend}
                      </span>
                    )}
                  </div>
                  <div style={{ color, fontSize: 26, fontWeight: 700 }}>{value}</div>
                  <div style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 12, marginTop: 4 }}>{label}</div>
                  {sub && <div style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 11, marginTop: 2 }}>{sub}</div>}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-5">
              ...
              <div className="rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <h3 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 14, fontWeight: 600, marginBottom: 4 }}>Compliance Trend (6m)</h3>
                <p style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 12, marginBottom: 14 }}>Platform-wide SLA compliance %</p>
                {sla.monthlyTrend.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center h-40" style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 12 }}>
              ...
              {/* Product compliance */}
              <div className="rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <h3 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 14, fontWeight: 600, marginBottom: 4 }}>By Product</h3>
                <p style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 12, marginBottom: 14 }}>SLA compliance per product</p>
            ...
            {/* Upcoming Breaches */}
            <div className="rounded-2xl" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(245,158,11,0.2)" }}>
              <div className="px-5 py-3 flex items-center gap-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                <Clock size={14} color="#F59E0B" />
                <h3 style={{ color: "var(--color-status-warning, #F59E0B)", fontSize: 13, fontWeight: 600 }}>Upcoming SLA Breaches</h3>
                <span className="ml-auto px-2 py-0.5 rounded-lg" style={{ background: "rgba(245,158,11,0.1)", color: "var(--color-status-warning, #F59E0B)", fontSize: 11 }}>Next 7 days</span>
              </div>
              <div className="p-4 flex flex-col gap-3">
                {sla.upcomingBreaches.length === 0 ? (
                  <div className="text-center py-6" style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 12 }}>
                  ...
                  <div key={f.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "rgba(245,158,11,0.05)", border: "1px solid rgba(245,158,11,0.1)" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 12 }} className="truncate">{f.title}</div>
                      <div style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 11 }}>Due {f.dueDate}</div>
                    </div>
