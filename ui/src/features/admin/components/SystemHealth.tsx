...
function useSystemHealth() {
  return useQuery<HealthData>({
    ...
    queryFn: async () => {
      ...
      // Normalize — đảm bảo services luôn là array và name luôn là string
      return {
        ...data,
...
const STATUS_STYLES: Record<string, { bg: string; color: string }> = {
  healthy: { bg: "rgba(16,185,129,0.1)", color: "var(--color-status-success, #10B981)" },
  degraded: { bg: "rgba(245,158,11,0.1)", color: "var(--color-status-warning, #F59E0B)" },
  down: { bg: "rgba(239,68,68,0.1)", color: "var(--color-status-error, #EF4444)" },
};
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-5 animate-pulse" style={{ background: "var(--color-bg-page, #0B1020)" }}>
      ...
      <div className="grid grid-cols-3 gap-3">
        {Array.from({ length: 9 }).map((_, i) => <div key={i} className="rounded-2xl h-28" style={{ background: "var(--color-bg-card, #151B2F)" }} />)}
      </div>
    </div>
  );
}
        ...
        return (
          <div className="flex-1 overflow-y-auto px-6 py-5" style={{ background: "var(--color-bg-page, #0B1020)" }}>
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 18, fontWeight: 700 }}>System Health</h2>
                <p style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 12 }}>Platform observability — {healthy} healthy · {degraded} degraded</p>
              </div>
            ...
            <div className="grid grid-cols-4 gap-4 mb-5">
              {[
                { label: "API Latency (P50)", value: `${health.latencyMs}ms`, color: "var(--color-status-success, #10B981)", sub: "Current" },
                { label: "Services Healthy", value: `${healthy}/${health.services.length}`, color: "var(--color-primary, #4F8CFF)", sub: `${degraded} degraded` },
                { label: "Platform Uptime", value: `${health.uptime}%`, color: "var(--color-status-success, #10B981)", sub: "30-day SLA" },
                ...
                <div key={s.label} className="rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  ...
                  <div style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 12, marginTop: 4 }}>{s.label}</div>
                  <div style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 11, marginTop: 2 }}>{s.sub}</div>
                </div>
              ))}
            </div>

            {/* Latency chart */}
            <div className="rounded-2xl p-5 mb-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
              <h3 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 13, fontWeight: 600, marginBottom: 12 }}>API Response Latency</h3>
            ...
            <div className="grid grid-cols-3 gap-3">
              {health.services.map(svc => {
                ...
                return (
                  <div key={svc.name} className="rounded-2xl p-4" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <Icon size={15} color={style.color} />
                        <span style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 13, fontWeight: 500 }}>{svc.name}</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <div key={m.label}>
                          <div style={{ color: "var(--color-text-faint, #4B5563)", fontSize: 9, letterSpacing: 0.5 }}>{m.label.toUpperCase()}</div>
                          <div style={{ color: "var(--color-text-secondary, #9CA3AF)", fontSize: 11, fontWeight: 600, marginTop: 1 }}>{m.value}</div>
                        </div>
                      ))}
                    </div>
                  </div>
