...
/**
 * Component for the sidebar "Nmap Results" menu item.
 *
 * Resolves recent Nmap scans via API:
...
export function LatestNmapRedirect() {
  ...
  const { data: scans, isLoading, isError } = useRecentScans('nmap');
    ...
    return (
      <div className="flex flex-col items-center justify-center flex-1 space-y-4 p-8">
        <div className="w-16 h-16 rounded-full bg-slate-800/50 flex items-center justify-center">
          <span className="text-slate-400 text-2xl">N</span>
        </div>
        <h2 className="text-lg font-medium text-white">No Nmap Scans Found</h2>
        <p className="text-sm text-slate-400 text-center max-w-sm">
          There are no completed scans in the system yet. Please create and run a new scan to view Nmap results.
        </p>
      </div>
    );
  }
  ...
  if (scans.length === 1) {
    ...
    if (scanId) {
      return <Navigate to={`/scans/${scanId}/results/nmap`} replace />;
    }
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-5" style={{ background: "var(--color-bg-page, #0B1020)" }}>
      <div className="mb-6">
        <h2 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 20, fontWeight: 700 }}>Select Nmap Scan Results</h2>
        <p style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 13, marginTop: 4 }}>
          Multiple Nmap scans were found. Please select one to view its detailed results.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 max-w-4xl">
        {scans.map((scan) => {
          ...
          return (
            <button
              key={scanId}
              onClick={() => navigate(`/scans/${scanId}/results/nmap`)}
