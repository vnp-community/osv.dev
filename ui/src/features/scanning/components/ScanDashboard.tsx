...
import { useWeeklyScanActivity } from "@/features/scanning/hooks/useScanStats";
...
function ScanDashboardSkeleton() {
  return (
    <div className="flex-1 overflow-y-auto px-6 py-5 animate-pulse" style={{ background: "var(--color-bg-page, #0B1020)" }}>
      ...
      <div className="rounded-2xl h-48 mb-5" style={{ background: "var(--color-bg-card, #151B2F)" }} />
      <div className="grid grid-cols-3 gap-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="rounded-2xl h-64" style={{ background: "var(--color-bg-card, #151B2F)" }} />
        ))}
      </div>
    </div>
  );
...
function ScanProgressBar({ progress, status }: { progress: number; status: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span style={{ color: "var(--color-primary, #4F8CFF)", fontSize: 12, fontWeight: 600 }}>{progress}%</span>
    </div>
  );
}
...
export function ScanDashboard({ onNewScan, onViewScan }: { onNewScan?: () => void; onViewScan?: () => void }) {
  ...
  // Stable weekly data — no Math.random()
        ...
        return (
          <div className="flex-1 overflow-y-auto px-6 py-5" style={{ background: "var(--color-bg-page, #0B1020)" }}>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 18, fontWeight: 700 }}>Scanning Operations Center</h1>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              {[
                { label: "Running Scans", value: String(runningScans.length), color: "var(--color-primary, #4F8CFF)", bg: "rgba(79,140,255,0.1)", icon: Activity, pulse: true },
                { label: "Completed Today", value: String(completedToday), color: "var(--color-status-success, #10B981)", bg: "rgba(16,185,129,0.1)", icon: CheckCircle },
                { label: "Failed Scans", value: String(failedScans), color: "var(--color-status-error, #EF4444)", bg: "rgba(239,68,68,0.1)", icon: XCircle },
                { label: "Total Assets", value: scansData.total?.toLocaleString() ?? "…", color: "var(--color-ai, #A78BFA)", bg: "rgba(167,139,250,0.1)", icon: Server },
              ].map(({ label, value, color, bg, icon: Icon, pulse }) => (
                <div key={label} className="rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  ...
                  <div style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 26, fontWeight: 700 }}>{value}</div>
                  <div style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 12, marginTop: 4 }}>{label}</div>
                </div>
              ))}
            </div>
            ...
            {runningScans.length > 0 && (
              <div className="rounded-2xl mb-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full animate-pulse" style={{ background: "#4F8CFF" }} />
                    <h3 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 14, fontWeight: 600 }}>Running Scans</h3>
                  </div>
                </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 14, fontWeight: 500 }}>{scan.name}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span style={{ color: "var(--color-status-warning, #F59E0B)", fontSize: 12 }}>{scan.findingCount} findings</span>
                          <div className="flex items-center gap-2">
                            <button
                              className="w-7 h-7 rounded-lg flex items-center justify-center"
                              style={{ background: "rgba(245,158,11,0.1)", color: "var(--color-status-warning, #F59E0B)", border: "none", cursor: "pointer" }}
                            ...
                            <button
                              className="w-7 h-7 rounded-lg flex items-center justify-center"
                              style={{ background: "rgba(239,68,68,0.1)", color: "var(--color-status-error, #EF4444)", border: "none", cursor: "pointer" }}
                      ...
                      <div className="flex items-center gap-4 mb-2">
                        <span style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 12, fontFamily: "monospace" }}>{scan.targets?.[0] ?? "—"}</span>
                      </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="col-span-1 rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <h3 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 14, fontWeight: 600, marginBottom: 16 }}>Scan Activity (7d)</h3>
              ...
              {/* Recent scans */}
              <div className="col-span-1 rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <h3 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Recent Scans</h3>
                <div className="flex flex-col gap-3">
                  {recentScans.map((s) => (
                    <div key={s.id} className="flex items-center gap-3 cursor-pointer" onClick={() => handleViewScan(s.id)}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 12 }} className="truncate">{s.name}</div>
                  ...
                  {recentScans.length === 0 && (
                    <div style={{ color: "var(--color-text-faint, #4B5563)", fontSize: 12, textAlign: "center", padding: "20px 0" }}>No recent scans</div>
                  )}
                </div>
              </div>
              ...
              {/* Quick actions */}
              <div className="col-span-1 rounded-2xl p-5" style={{ background: "var(--color-bg-card, #151B2F)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <h3 style={{ color: "var(--color-text-primary, #E5E7EB)", fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Quick Actions</h3>
                <div className="flex flex-col gap-3">
                  {[
                    { label: "New NMAP Scan", color: "var(--color-primary, #4F8CFF)", desc: "Network discovery & port scan" },
                    { label: "New ZAP Scan", color: "var(--color-severity-high, #F97316)", desc: "Web application security test" },
                    { label: "View Scan History", color: "var(--color-ai, #A78BFA)", desc: "Browse all past scans" },
                  ].map((action) => (
                    <button
                      key={action.label}
                    ...
                    >
                      <div>
                        ...
                        <div style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 10, marginTop: 2 }}>{action.desc}</div>
                      </div>
                ...
                <div className="mt-4 pt-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span style={{ color: "var(--color-text-secondary, #9CA3AF)", fontSize: 11, fontWeight: 600 }}>SCAN STATS</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-lg p-2" style={{ background: "rgba(79,140,255,0.08)" }}>
                      <div style={{ color: "var(--color-primary, #4F8CFF)", fontSize: 18, fontWeight: 700 }}>{allScans.length}</div>
                      <div style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 10 }}>Total Scans</div>
                    </div>
                    <div className="rounded-lg p-2" style={{ background: "rgba(16,185,129,0.08)" }}>
                      <div style={{ color: "var(--color-status-success, #10B981)", fontSize: 18, fontWeight: 700 }}>{completedToday}</div>
                      <div style={{ color: "var(--color-text-muted, #6B7280)", fontSize: 10 }}>Completed</div>
                    </div>
                  </div>
                </div>
              </div>
