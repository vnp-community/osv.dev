...
const ENV_STYLES: Record<string, { bg: string; color: string }> = {
  ...
  Development:   { bg: "rgba(79,140,255,0.1)",   color: "#4F8CFF" },
  Infrastructure:{ bg: "rgba(167,139,250,0.1)",  color: "#A78BFA" },
  Network:       { bg: "rgba(245,158,11,0.1)",   color: "#F59E0B" },
};
    ...
    <div className="flex flex-col flex-1 overflow-hidden" style={{ background: "#0B1020" }}>
      ...
      <div className="px-6 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 style={{ color: "#E5E7EB", fontSize: 18, fontWeight: 700 }}>Asset Inventory</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.08)", color: "#9CA3AF", fontSize: 12 }}
          >
                ...
                <tbody>
                  {filtered.map((a) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3"><span style={{ color: "#4F8CFF", fontSize: 12, fontFamily: "monospace" }}>{a.ip}</span></td>
                        <td className="px-4 py-3"><span style={{ color: "#E5E7EB", fontSize: 12 }}>{a.hostname}</span></td>
                        <td className="px-4 py-3"><span style={{ color: "#9CA3AF", fontSize: 12 }}>{a.os}</span></td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1 flex-wrap">
                            {ports.slice(0, 4).map((p) => (
                              <span key={p} className="px-1.5 py-0.5 rounded" style={{ background: "rgba(79,140,255,0.1)", color: "#4F8CFF", fontSize: 10 }}>{p}</span>
                            ))}
                            {ports.length > 4 && <span style={{ color: "#6B7280", fontSize: 10 }}>+{ports.length - 4}</span>}
                          </div>
                        </td>
                        <td className="px-4 py-3"><span style={{ color: "#6B7280", fontSize: 12 }}>{lastScan}</span></td>
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            {a.activeFindingCount > 0 ? (
                              <span style={{ color: "#F97316", fontSize: 11 }}>{a.activeFindingCount} open</span>
                            ) : (
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1 flex-wrap">
                            {tags.slice(0, 2).map((t) => (
                              <span key={t} className="px-1.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.06)", color: "#9CA3AF", fontSize: 10 }}>{t}</span>
                            ))}
                          </div>
                        </td>
                        <td className="px-4 py-3">
