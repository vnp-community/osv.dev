    ...
    return (
      <div className="flex-1 flex items-center justify-center" style={{ background: "#0B1020" }}>
        <div className="text-center">
          <p style={{ color: "#EF4444", fontSize: 14 }}>Scan not found</p>
          <button onClick={handleBack} style={{ color: "#4F8CFF", background: "none", border: "none", cursor: "pointer", marginTop: 8, fontSize: 13 }}>← Back to Scans</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden" style={{ background: "#0B1020" }}>
      ...
      <div
        ...
        style={{ background: "#0F1629", borderBottom: "1px solid rgba(255,255,255,0.06)" }}
      >
        <div className="flex items-center gap-4">
          <button onClick={handleBack} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "rgba(255,255,255,0.05)", color: "#9CA3AF", border: "none", cursor: "pointer" }}>
            <ArrowLeft size={15} />
          </button>
          <div>
            <div className="flex items-center gap-3">
              <h2 style={{ color: "#E5E7EB", fontSize: 16, fontWeight: 700 }}>{scan.name}</h2>
              <span
            ...
            <div className="flex items-center gap-4 mt-1">
              <span style={{ color: "#6B7280", fontSize: 12 }}>{scan.type.toUpperCase()} · {scan.targets?.[0] ?? "—"}</span>
              <span style={{ color: "#6B7280", fontSize: 12 }}>{scan.id}</span>
              <span style={{ color: "#4F8CFF", fontSize: 12, fontFamily: "monospace" }}>
          ...
          {isActive && (
            <>
              <button
                ...
                style={{ background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)", color: "#F59E0B", fontSize: 13, cursor: "pointer" }}
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl"
                style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", color: "#EF4444", fontSize: 13, cursor: "pointer", opacity: cancelScan.isPending ? 0.6 : 1 }}
              >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col flex-1 overflow-hidden p-5 gap-4">
          ...
          <div className="rounded-2xl p-5" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="flex items-center justify-between mb-3">
              <span style={{ color: "#E5E7EB", fontSize: 14, fontWeight: 600 }}>Overall Progress</span>
              <div className="flex items-center gap-4">
                <span style={{ color: "#F59E0B", fontSize: 13 }}>{findingsFound} findings</span>
                <span style={{ color: "#4F8CFF", fontSize: 18, fontWeight: 700 }}>{overallProgress}%</span>
              </div>
            </div>
            ...
            {progress?.currentTarget && (
              <div style={{ color: "#6B7280", fontSize: 12 }}>
                Current target: <span style={{ color: "#4F8CFF", fontFamily: "monospace" }}>{progress.currentTarget}</span>
              </div>
            )}
            {sseStatus === "connecting" && (
              <div style={{ color: "#F59E0B", fontSize: 11, marginTop: 4 }}>Connecting to live stream...</div>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 flex-1 overflow-hidden">
            {/* Scan metadata */}
            <div className="rounded-2xl flex flex-col overflow-hidden" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                <Activity size={14} color="#4F8CFF" />
                <span style={{ color: "#E5E7EB", fontSize: 13, fontWeight: 600 }}>Scan Info</span>
              </div>
              <div className="flex-1 overflow-y-auto p-4">
                {[
                  ...
                  <div key={label} className="flex justify-between py-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <span style={{ color: "#E5E7EB", fontSize: 12, fontFamily: label === "Scan ID" || label === "Targets" ? "monospace" : undefined }}>
                      {value}
                    </span>
                  </div>
                ))}
            ...
            <div className="rounded-2xl flex flex-col overflow-hidden" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                ...
                <span style={{ color: "#E5E7EB", fontSize: 13, fontWeight: 600 }}>Live Stats</span>
                {isActive && <div className="w-1.5 h-1.5 rounded-full animate-pulse ml-auto" style={{ background: "#10B981" }} />}
              </div>
              <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
                {[
                  { label: "Progress", value: `${overallProgress}%`, color: "#4F8CFF" },
                  { label: "Findings Found", value: String(findingsFound), color: "#F97316" },
            ...
            <div className="rounded-2xl flex flex-col overflow-hidden" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                ...
                <span style={{ color: "#E5E7EB", fontSize: 13, fontWeight: 600 }}>Targets</span>
                <span className="ml-auto px-2 py-0.5 rounded-full" style={{ background: "rgba(79,140,255,0.1)", color: "#4F8CFF", fontSize: 11 }}>
              ...
              <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
                {(scan.targets ?? []).map((t) => (
                  <div key={t} className="flex items-center gap-3 rounded-xl p-3" style={{ background: "rgba(255,255,255,0.03)" }}>
                    ...
                    <span style={{ color: "#E5E7EB", fontSize: 12, fontFamily: "monospace" }}>{t}</span>
                  </div>
                ))}
                {(!scan.targets || scan.targets.length === 0) && (
                  <div style={{ color: "#4B5563", fontSize: 12, textAlign: "center", padding: "20px 0" }}>No targets defined</div>
                )}
              </div>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <span style={{ color: "#E5E7EB", fontSize: 13, fontWeight: 600 }}>Live Logs</span>
            {isActive && <div className="w-1.5 h-1.5 rounded-full animate-pulse ml-auto" style={{ background: "#10B981" }} />}
          </div>
