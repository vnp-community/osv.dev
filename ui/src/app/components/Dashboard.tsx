...
function KPICard({
  ...
  return (
    ...
    >
      ...
      <div style={{ color: "#E5E7EB", fontSize: 28, fontWeight: 700, lineHeight: 1 }}>{value}</div>
      <div style={{ color: "#6B7280", fontSize: 12, marginTop: 6 }}>{label}</div>
      ...
      <div className="rounded-xl p-3" style={{ background: "#1E2A45", border: "1px solid rgba(255,255,255,0.1)", fontSize: 12 }}>
        ...
        {payload.map((p: any) => (
          <div key={p.name} className="flex items-center gap-2">
            ...
            <span style={{ color: "#E5E7EB" }}>{p.name}: <b>{p.value}</b></span>
          </div>
        ))}
      </div>
    );
...
function DashboardContent({ data, period, onPeriodChange }: {
  ...
  const severityChartItems = [
    { name: "Critical", value: severity_distribution.critical, color: "#EF4444" },
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ background: "#0B1020", padding: "24px" }}>
      {/* Page header */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 style={{ color: "#E5E7EB", fontWeight: 700, fontSize: 20 }}>Executive Security Dashboard</h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {/* Risk Trend */}
        <div className="col-span-2 rounded-2xl p-5" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 style={{ color: "#E5E7EB", fontSize: 14, fontWeight: 600 }}>Risk Trend</h3>
              <p style={{ color: "#6B7280", fontSize: 12 }}>Findings over time by severity</p>
            </div>
        ...
        <div className="rounded-2xl p-5" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="mb-4">
            <h3 style={{ color: "#E5E7EB", fontSize: 14, fontWeight: 600 }}>Severity Distribution</h3>
            <p style={{ color: "#6B7280", fontSize: 12 }}>Total: {(severity_distribution.total).toLocaleString()} findings</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 mt-2">
            {severityChartItems.map((d) => (
              <div key={d.name} className="flex items-center gap-2">
                ...
                <div>
                  <div style={{ color: "#E5E7EB", fontSize: 12, fontWeight: 600 }}>{d.value}</div>
                  <div style={{ color: "#6B7280", fontSize: 10 }}>{d.name}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="col-span-1 rounded-2xl p-5" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-center justify-between mb-4">
            <h3 style={{ color: "#E5E7EB", fontSize: 14, fontWeight: 600 }}>Product Security</h3>
            <button style={{ color: "#4F8CFF", fontSize: 12, background: "none", border: "none", cursor: "pointer" }}>View all</button>
          </div>
            ...
            {product_grades.map((p) => (
              <div key={p.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div style={{ color: "#E5E7EB", fontSize: 13, fontWeight: 500 }}>{p.name}</div>
                  <div className="flex gap-2 mt-0.5">
                    <span style={{ color: "#EF4444", fontSize: 11 }}>{p.critical_count} crit</span>
                    <span style={{ color: "#F97316", fontSize: 11 }}>{p.high_count} high</span>
                  </div>
                </div>
        ...
        <div className="rounded-2xl p-5" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 style={{ color: "#E5E7EB", fontSize: 14, fontWeight: 600 }}>KEV Alerts</h3>
            </div>
            <span className="px-2 py-1 rounded-lg" style={{ background: "rgba(239,68,68,0.1)", color: "#EF4444", fontSize: 11 }}>
          ...
          <div className="flex flex-col gap-3">
            {kev_alerts.map((k) => (
              <div key={k.cve_id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.1)" }}>
                ...
                <div className="flex-1 min-w-0">
                  <div style={{ color: "#EF4444", fontSize: 12, fontWeight: 600 }}>{k.cve_id}</div>
                  <div style={{ color: "#6B7280", fontSize: 11 }}>{k.vendor} · {k.product}</div>
                </div>
                <span style={{ color: "#4B5563", fontSize: 10 }}>{new Date(k.date_added).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
          <div className="mt-4 pt-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                <div style={{ color: "#EF4444", fontSize: 16, fontWeight: 700 }}>{kev_alerts.length}</div>
                <div style={{ color: "#6B7280", fontSize: 10 }}>In Platform</div>
              </div>
              <div>
                <div style={{ color: "#F59E0B", fontSize: 16, fontWeight: 700 }}>{kev_alerts.filter(k => k.is_ransomware).length}</div>
                <div style={{ color: "#6B7280", fontSize: 10 }}>Ransomware</div>
              </div>
              <div>
                <div style={{ color: "#4F8CFF", fontSize: 16, fontWeight: 700 }}>{kpis.critical_findings}</div>
                <div style={{ color: "#6B7280", fontSize: 10 }}>Unmitigated</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-2xl p-4" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
            <h3 style={{ color: "#E5E7EB", fontSize: 13, fontWeight: 600, marginBottom: 12 }}>Recent Scans</h3>
            <div className="flex flex-col gap-2">
              {recent_scans.map((s) => (
                <div key={s.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div style={{ color: "#E5E7EB", fontSize: 12 }}>{s.name}</div>
                    <div style={{ color: "#6B7280", fontSize: 10 }}>{s.type.toUpperCase()} · {s.started_at ? new Date(s.started_at).toLocaleTimeString() : "—"}</div>
                  </div>
          ...
          <div className="rounded-2xl p-4" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="flex items-center justify-between mb-3">
              <h3 style={{ color: "#E5E7EB", fontSize: 13, fontWeight: 600 }}>SLA Breaches</h3>
              <span className="px-2 py-0.5 rounded-lg" style={{ background: "rgba(239,68,68,0.1)", color: "#EF4444", fontSize: 11 }}>
            ...
            <div className="flex flex-col gap-2">
              {sla_breaches.map((b) => (
                <div key={b.finding_id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div style={{ color: "#E5E7EB", fontSize: 11 }} className="truncate">{b.title}</div>
      ...
      <div className="flex items-center justify-end">
        <button className="flex items-center gap-1 text-sm" style={{ color: "#4F8CFF", background: "none", border: "none", cursor: "pointer", fontSize: 12 }}>
