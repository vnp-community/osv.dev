...
const STEPS = [
  { id: 1, icon: Package, color: "#4F8CFF", title: "Create Your First Product", desc: "Define the application or service you want to secure", action: "Create Product" },
  { id: 2, icon: Server, color: "#10B981", title: "Add Assets", desc: "Register the servers, IPs, or URLs to protect", action: "Add Asset" },
  { id: 3, icon: Scan, color: "#A78BFA", title: "Run Your First Scan", desc: "Launch a network or web application scan", action: "Start Scan" },
  { id: 4, icon: AlertTriangle, color: "#F97316", title: "Review Findings", desc: "Triage the vulnerabilities discovered by the scan", action: "Review Findings" },
  { id: 5, icon: FileText, color: "#F59E0B", title: "Generate a Report", desc: "Create an executive or technical security report", action: "Generate Report" },
];
...
export function OnboardingExperience({ onNavigate }: { onNavigate?: (s: string) => void }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-8" style={{ background: "#0B1020" }}>
      {/* Hero */}
      <div className="text-center mb-10">
        ...
        <h1 style={{ color: "#E5E7EB", fontSize: 28, fontWeight: 800, letterSpacing: -0.5 }}>Welcome to OSV Platform</h1>
        <p style={{ color: "#9CA3AF", fontSize: 15, marginTop: 8, maxWidth: 480, margin: "8px auto 0" }}>
          Let's get you set up. Follow these steps to start securing your infrastructure in minutes.
        </p>
        {/* Progress bar */}
        <div className="max-w-sm mx-auto mt-6">
          <div className="flex justify-between mb-2">
            <span style={{ color: "#6B7280", fontSize: 12 }}>Setup Progress</span>
            <span style={{ color: "#4F8CFF", fontSize: 12, fontWeight: 600 }}>{completed.length}/{STEPS.length} completed</span>
          </div>
            ...
            return (
              <div key={step.id} className="relative mb-5 last:mb-0">
                ...
                <div className="absolute -left-8 w-7 h-7 rounded-full flex items-center justify-center" style={{ background: done ? step.color : "rgba(255,255,255,0.08)", border: done ? "none" : "1px solid rgba(255,255,255,0.12)" }}>
                  {done ? <Check size={13} color="white" /> : <span style={{ color: "#6B7280", fontSize: 11, fontWeight: 600 }}>{step.id}</span>}
                </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div style={{ color: "#6B7280", fontSize: 12, marginTop: 2 }}>{step.desc}</div>
                      </div>
                    </div>
                    {!done ? (
                      <button
      ...
      <div className="max-w-2xl mx-auto">
        <div style={{ color: "#6B7280", fontSize: 11, fontWeight: 600, letterSpacing: 1, marginBottom: 12 }}>PRO TIPS</div>
          ...
          {TIPS.map(tip => (
            <div key={tip.title} className="rounded-xl p-4" style={{ background: "rgba(79,140,255,0.05)", border: "1px solid rgba(79,140,255,0.12)" }}>
              <div className="flex items-start gap-2">
                ...
                <div>
                  <div style={{ color: "#E5E7EB", fontSize: 12, fontWeight: 500 }}>{tip.title}</div>
                  <div style={{ color: "#6B7280", fontSize: 11, marginTop: 3 }}>{tip.desc}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        {progress === 100 && (
          <div className="mt-6 rounded-2xl p-6 text-center" style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.25)" }}>
            ...
            <h3 style={{ color: "#10B981", fontSize: 18, fontWeight: 700 }}>Setup Complete!</h3>
