        ...
        {rowVirtualizer.getVirtualItems().map((virtualItem) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ color: "#6B7280", fontSize: 11, width: 90, flexShrink: 0 }}>{f.id}</span>
              <span style={{ color: "#E5E7EB", fontSize: 12, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} onClick={() => navigate(`/findings/${f.id}`)}>{f.title}</span>
              <span style={{ color: "#4F8CFF", fontSize: 12, fontFamily: "monospace", width: 120, flexShrink: 0 }}>{f.cveId ?? "—"}</span>
...
export function FindingsList() {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden" style={{ background: "#0B1020" }}>
      {/* Header */}
      <div className="px-6 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", background: "#0F1629" }}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 style={{ color: "#E5E7EB", fontSize: 18, fontWeight: 700 }}>Findings</h2>
          ...
          {selected.length > 0 && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl" style={{ background: "rgba(79,140,255,0.08)", border: "1px solid rgba(79,140,255,0.2)" }}>
              <span style={{ color: "#4F8CFF", fontSize: 13 }}>{selected.length} selected</span>
              {[
                { label: "Close", icon: CheckCircle, color: "#10B981", action: () => selected.forEach(id => updateFinding({ id, status: "mitigated" as FindingStatus })) },
                { label: "Reopen", icon: RotateCcw, color: "#F59E0B", action: () => selected.forEach(id => updateFinding({ id, status: "active" as FindingStatus })) },
                { label: "Accept Risk", icon: AlertTriangle, color: "#A78BFA", action: () => selected.forEach(id => updateFinding({ id, status: "risk_accepted" as FindingStatus })) },
                { label: "Tag", icon: Tag, color: "#6B7280", action: () => {} },
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              style={{ background: "#1A2236", border: "1px solid rgba(255,255,255,0.08)", color: "#E5E7EB", fontSize: 12, width: 220 }}
            />
          </div>
          ...
          <select
            ...
            style={{ background: "#1A2236", border: "1px solid rgba(255,255,255,0.08)", color: "#9CA3AF", fontSize: 12 }}
          >
              ...
              <div className="flex-1 overflow-hidden">
                ...
                <div className="flex items-center gap-2 px-4 py-2.5" style={{ background: "#0D1525", borderBottom: "1px solid rgba(255,255,255,0.06)", position: "sticky", top: 0, zIndex: 5 }}>
                  ...
                  {["ID", "Title", "CVE ID", "Severity", "Status", "SLA"].map((h) => (
                    <span key={h} style={{ color: "#6B7280", fontSize: 11, fontWeight: 600, letterSpacing: 0.5, flex: h === "Title" ? 1 : undefined, width: h === "ID" ? 90 : h === "CVE ID" ? 120 : undefined, flexShrink: 0 }}>{h}</span>
                  ))}
                </div>
                ...
                <thead style={{ position: "sticky", top: 0, background: "#0D1525", zIndex: 5 }}>
                  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    {["ID", "Title", "CVE", "Severity", "Product", "Asset", "EPSS", "Status", "SLA", "AI Triage", ""].map((h) => (
                      <th key={h} className="px-3 py-3 text-left" style={{ color: "#6B7280", fontSize: 11, fontWeight: 600, letterSpacing: 0.5 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {findings.map((f) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5"><span style={{ color: "#6B7280", fontSize: 11 }}>{f.id}</span></td>
                      ...
                      <td className="px-3 py-2.5"><span style={{ color: "#4F8CFF", fontSize: 12, fontFamily: "monospace" }}>{f.cveId ?? "—"}</span></td>
                      <td className="px-3 py-2.5"><SeverityBadge severity={f.severity} /></td>
                      <td className="px-3 py-2.5"><span style={{ color: "#9CA3AF", fontSize: 12 }}>{f.productName ?? "—"}</span></td>
                      <td className="px-3 py-2.5"><span style={{ color: "#6B7280", fontSize: 12, fontFamily: "monospace" }}>{f.assetIp ?? "—"}</span></td>
                      ...
                      <td className="px-3 py-2.5">
                        <button
                          ...
                          style={{ background: "rgba(255,255,255,0.05)", color: "#6B7280", border: "none", cursor: "pointer" }}
                        >
                          <Eye size={12} />
                        </button>
                      </td>
      ...
      {/* Pagination */}
      <div className="flex items-center justify-between px-6 py-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)", background: "#0F1629" }}>
