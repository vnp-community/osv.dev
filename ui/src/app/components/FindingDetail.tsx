      ...
      <div className="px-6 py-4" style={{ background: "#0F1629", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex items-center gap-3 flex-1">
            <span style={{ color: "#6B7280", fontSize: 13 }}>{finding.id}</span>
            <ChevronRight size={13} color="#4B5563" />
            <h2 style={{ color: "#E5E7EB", fontSize: 16, fontWeight: 700 }}>{finding.title}</h2>
            <span className="px-2.5 py-1 rounded-lg" style={{ background: severityColor + "25", color: severityColor, fontSize: 13, fontWeight: 700 }}>
              {finding.severity}
            </span>
            {finding.isKEV && (
              <span className="px-2 py-0.5 rounded" style={{ background: "rgba(239,68,68,0.1)", color: "#EF4444", fontSize: 12 }}>CISA KEV</span>
            )}
          </div>
          {/* Actions */}
          <div className="flex items-center gap-2">
            {([
              { label: "Mitigate", status: "mitigated" as FindingStatus, color: "#10B981", bg: "rgba(16,185,129,0.1)", border: "rgba(16,185,129,0.25)" },
              { label: "Accept Risk", status: "risk_accepted" as FindingStatus, color: "#A78BFA", bg: "rgba(167,139,250,0.1)", border: "rgba(167,139,250,0.25)" },
              { label: "False Positive", status: "false_positive" as FindingStatus, color: "#6B7280", bg: "rgba(107,114,128,0.1)", border: "rgba(107,114,128,0.2)" },
      ...
      <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-3 gap-5">
            ...
            <div className="col-span-2 flex flex-col gap-5">
              ...
              <div className="grid grid-cols-4 gap-4">
                {[
                  ...
                  { label: "EPSS Score", value: finding.epssScore ? `${(finding.epssScore * 100).toFixed(1)}%` : "N/A", color: "#EF4444", sub: "Exploit prediction" },
                  ...
                  { label: "Status", value: finding.status, color: "#4F8CFF", sub: "Current status" },
                ].map((m) => (
                  <div key={m.label} className="rounded-2xl p-4" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
                    ...
                    <div style={{ color: "#6B7280", fontSize: 11, marginTop: 3 }}>{m.sub}</div>
                  </div>
                ))}
              </div>
              ...
              {finding.description && (
                <div className="rounded-2xl p-5" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
                  <div style={{ color: "#9CA3AF", fontSize: 12, fontWeight: 600, marginBottom: 12 }}>DESCRIPTION</div>
                  <p style={{ color: "#E5E7EB", fontSize: 13, lineHeight: 1.7 }}>{finding.description}</p>
                </div>
              )}

              {/* AI Section */}
              <div className="rounded-2xl p-5" style={{ background: "#151B2F", border: "1px solid rgba(167,139,250,0.2)" }}>
                <div className="flex items-center gap-2 mb-4">
                  <Brain size={16} color="#A78BFA" />
                  <span style={{ color: "#A78BFA", fontSize: 14, fontWeight: 600 }}>AI Triage Analysis</span>
                  <span className="ml-auto px-2 py-0.5 rounded-lg" style={{ background: "rgba(16,185,129,0.1)", color: "#10B981", fontSize: 12 }}>
                ...
                {finding.aiTriageResult?.actions && finding.aiTriageResult.actions.length > 0 && (
                  <>
                    <div style={{ color: "#9CA3AF", fontSize: 12, fontWeight: 600, marginBottom: 8 }}>SUGGESTED ACTIONS</div>
                    {finding.aiTriageResult.actions.map((action, i) => (
                      <div key={i} className="flex items-start gap-2 mb-2">
                        <CheckCircle size={13} color="#10B981" style={{ marginTop: 2, flexShrink: 0 }} />
                        <span style={{ color: "#E5E7EB", fontSize: 12 }}>{action}</span>
                      </div>
                    ))}
                  </>
                )}
            ...
            <div className="flex flex-col gap-4">
              <div className="rounded-2xl p-4" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div style={{ color: "#9CA3AF", fontSize: 11, fontWeight: 600, marginBottom: 10 }}>DETAILS</div>
                {[
              ...
              {finding.jiraUrl && (
                <div className="rounded-2xl p-4" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
                  <div style={{ color: "#9CA3AF", fontSize: 11, fontWeight: 600, marginBottom: 10 }}>REFERENCES</div>
                  <div className="flex items-center gap-2 py-2">
                    <ExternalLink size={11} color="#4F8CFF" />
                    <a href={finding.jiraUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#4F8CFF", fontSize: 12 }}>
        ...
        {activeTab === "Evidence" && (
          <div className="flex flex-col gap-5">
              ...
              <div className="rounded-2xl p-5" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div style={{ color: "#9CA3AF", fontSize: 12, fontWeight: 600, marginBottom: 12 }}>SCAN EVIDENCE</div>
            ...
            <div className="flex flex-col gap-4">
              {(finding.vexJustification ? [
                ...
                <div key={i} className="flex items-start gap-4">
                  ...
                  <div className="flex-1 rounded-xl p-3" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
                    <div className="flex items-center justify-between mb-1">
                      <span style={{ color: "#E5E7EB", fontSize: 13, fontWeight: 500 }}>{entry.user}</span>
                      <span style={{ color: "#6B7280", fontSize: 11 }}>{entry.time ? new Date(entry.time).toLocaleString() : "—"}</span>
                    </div>
                    <p style={{ color: "#9CA3AF", fontSize: 12 }}>{entry.action}</p>
                  </div>
                </div>
              ))}
              {(!finding.vexJustification) && (
                <div style={{ color: "#6B7280", fontSize: 13 }}>No audit trail available.</div>
              )}
            </div>
          </div>
        )}
            ...
            <div className="relative pl-6">
              ...
              {(finding.vexJustification ? [
                ...
                <div key={i} className="relative mb-5">
                  ...
                  <div className="rounded-xl p-3" style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.07)" }}>
                    <div style={{ color: "#6B7280", fontSize: 11, marginBottom: 4 }}>{entry.time ? new Date(entry.time).toLocaleString() : "—"}</div>
                    <div style={{ color: "#9CA3AF", fontSize: 12 }}><span style={{ color: "#E5E7EB" }}>{entry.user}</span> — {entry.action}</div>
                  </div>
                </div>
              ))}
            </div>
...
function FindingDetailSkeleton() {
  return (
    <div className="flex flex-col flex-1 overflow-hidden animate-pulse" style={{ background: "#0B1020" }}>
      <div className="h-24 mb-4" style={{ background: "#0F1629" }} />
