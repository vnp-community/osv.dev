...
function FieldError({ message }: { message?: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5 mt-1.5">
      ...
      <span style={{ color: "#EF4444", fontSize: 11 }}>{message}</span>
    </div>
  );
}
...
export function ScanWizard({ onClose }: { onClose?: () => void; onStart?: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 overflow-hidden" style={{ background: "#0B1020" }}>
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", background: "#0F1629" }}>
        <div className="flex items-center gap-3">
          <button type="button" onClick={handleBack} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "rgba(255,255,255,0.05)", color: "#9CA3AF", border: "none", cursor: "pointer" }}>
            <ArrowLeft size={15} />
          </button>
          <div>
            <h2 style={{ color: "#E5E7EB", fontSize: 16, fontWeight: 700 }}>Create New Scan</h2>
            <p style={{ color: "#6B7280", fontSize: 12 }}>Step {step} of 4 — {STEP_LABELS[step - 1]}</p>
          </div>
        </div>
        <button type="button" onClick={handleBack} style={{ color: "#6B7280", background: "none", border: "none", cursor: "pointer" }}>
          <X size={18} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6">
        ...
        {step === 1 && (
          <div>
            <h3 style={{ color: "#E5E7EB", fontSize: 16, fontWeight: 600, marginBottom: 6 }}>Choose Scan Type</h3>
            <p style={{ color: "#6B7280", fontSize: 13, marginBottom: 24 }}>Select the scanning engine that matches your target.</p>
            <Controller
              name="type"
              control={control}
              render={({ field }) => (
                  ...
                  {SCAN_TYPES.map((type) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ color: "#E5E7EB", fontSize: 15, fontWeight: 600 }}>{type.name}</div>
                        <div style={{ color: type.color, fontSize: 12, marginBottom: 8 }}>{type.subtitle}</div>
                        <p style={{ color: "#9CA3AF", fontSize: 12, lineHeight: 1.6, marginBottom: 12 }}>{type.description}</p>
        ...
        {step === 2 && (
          <div className="max-w-xl">
            <h3 style={{ color: "#E5E7EB", fontSize: 16, fontWeight: 600, marginBottom: 6 }}>Scan Name & Targets</h3>
            <p style={{ color: "#6B7280", fontSize: 13, marginBottom: 24 }}>Give your scan a name and define the targets.</p>

            {/* Scan name */}
            <div className="mb-5">
              <label style={{ color: "#9CA3AF", fontSize: 13, display: "block", marginBottom: 8 }}>
                Scan Name <span style={{ color: "#EF4444" }}>*</span>
              </label>
              <input
                {...register("name")}
            ...
            <div className="mb-5">
              <label style={{ color: "#9CA3AF", fontSize: 13, display: "block", marginBottom: 8 }}>
                {selectedType?.id === "zap" ? "Target URLs" : "IP Ranges / Hostnames"} <span style={{ color: "#EF4444" }}>*</span>
              </label>
              <textarea
                {...register("targetsRaw")}
            ...
            <div className="rounded-xl p-4" style={{ background: "rgba(79,140,255,0.06)", border: "1px solid rgba(79,140,255,0.15)" }}>
              <div style={{ color: "#4F8CFF", fontSize: 12, fontWeight: 600, marginBottom: 4 }}>Format Examples</div>
              <div style={{ color: "#6B7280", fontSize: 11, fontFamily: "monospace", lineHeight: 1.8 }}>
        ...
        {step === 3 && (
          <div className="max-w-2xl">
            <h3 style={{ color: "#E5E7EB", fontSize: 16, fontWeight: 600, marginBottom: 6 }}>Scan Configuration</h3>
            <p style={{ color: "#6B7280", fontSize: 13, marginBottom: 24 }}>Fine-tune scan options for better results.</p>
            <div className="grid grid-cols-2 gap-6">
              <div>
                <div style={{ color: "#9CA3AF", fontSize: 12, fontWeight: 600, marginBottom: 12 }}>SCAN PROFILE</div>
                <Controller
                  name="scanProfile"
                  control={control}
                  render={({ field }) => (
              ...
              <div>
                <div style={{ color: "#9CA3AF", fontSize: 12, fontWeight: 600, marginBottom: 12 }}>PERFORMANCE</div>
                <div className="mb-4">
                  <label style={{ color: "#E5E7EB", fontSize: 13, display: "block", marginBottom: 8 }}>Port Range</label>
                  <input
                    {...register("portRange")}
                    className="w-full rounded-xl px-4 py-2.5 outline-none"
                    style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.09)", color: "#E5E7EB", fontSize: 13, fontFamily: "monospace" }}
                  />
                  <FieldError message={errors.portRange?.message} />
                </div>
                <div>
                  <label style={{ color: "#E5E7EB", fontSize: 13, display: "block", marginBottom: 8 }}>Scan Frequency</label>
                  <Controller
                    name="frequency"
                    control={control}
                    render={({ field }) => (
                      <select
                        {...field}
                        className="w-full rounded-xl px-4 py-2.5 outline-none"
                        style={{ background: "#151B2F", border: "1px solid rgba(255,255,255,0.09)", color: "#E5E7EB", fontSize: 13 }}
                      >
        ...
        {step === 4 && selectedType && (
          <div className="max-w-xl">
            <h3 style={{ color: "#E5E7EB", fontSize: 16, fontWeight: 600, marginBottom: 6 }}>Review & Launch</h3>
            <p style={{ color: "#6B7280", fontSize: 13, marginBottom: 24 }}>Confirm your scan configuration before starting.</p>
            ...
            <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.09)" }}>
              <div className="p-5" style={{ background: `${selectedType.color}10`, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div style={{ color: "#E5E7EB", fontSize: 15, fontWeight: 600 }}>{selectedType.name}</div>
              ...
              {[
                ...
                <div key={label} className="flex px-5 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                  <span style={{ color: "#6B7280", fontSize: 13, width: 160 }}>{label}</span>
                  <span style={{ color: "#E5E7EB", fontSize: 13, fontFamily: mono ? "monospace" : undefined }}>{value}</span>
                </div>
              ))}
            </div>

            <div className="mt-5 rounded-xl p-4" style={{ background: "rgba(79,140,255,0.08)", border: "1px solid rgba(79,140,255,0.2)" }}>
              <div style={{ color: "#4F8CFF", fontSize: 12 }}>
      ...
      {/* Footer */}
      <div className="flex items-center justify-between px-8 py-4" style={{ borderTop: "1px solid rgba(255,255,255,0.06)", background: "#0F1629" }}>
        <button
          type="button"
          onClick={handleBack}
