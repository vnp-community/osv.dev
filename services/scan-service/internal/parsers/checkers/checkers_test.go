...
import (
	...
	"github.com/osv/scan-service/internal/parsers/checkers"
...
func TestCheckerDef_EmptyName(t *testing.T) {
	def := checkers.CheckerDef{
		Name:          "",
		VendorProduct: []parserentity.VendorProduct{{Vendor: "x", Product: "y"}},
	}
	_, err := def.Build()
	if err == nil {
...
func TestCheckerDef_UppercaseVendor(t *testing.T) {
	def := checkers.CheckerDef{
		Name:          "test",
		VendorProduct: []parserentity.VendorProduct{{Vendor: "OpenSSL", Product: "openssl"}},
	}
	_, err := def.Build()
	if err == nil {
...
func TestCheckerDef_InvalidRegex(t *testing.T) {
	def := checkers.CheckerDef{
		...
		VendorProduct:    []parserentity.VendorProduct{{Vendor: "a", Product: "b"}},
	}
	_, err := def.Build()
	if err == nil {
...
func buildChecker(t *testing.T, def checkers.CheckerDef) *parserentity.Checker {
	t.Helper()
	c, err := def.Build()
	if err != nil {
...
func findChecker(t *testing.T, name string) *parserentity.Checker {
	t.Helper()
