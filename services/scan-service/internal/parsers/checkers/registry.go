...
// BuildAll compiles all registered checker definitions into *parserentity.Checker instances.
// Must be called once at startup after all init() functions have run.
// Returns an error if any definition is invalid.
func BuildAll() ([]*parserentity.Checker, error) {
	checkers := make([]*parserentity.Checker, 0, len(globalDefs))
