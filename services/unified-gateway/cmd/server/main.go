// Package main is the entrypoint for the unified-gateway.
...
import (
	...
	"github.com/osv/unified-gateway/internal/health"
)
...
func main() {
	...
	go func() {
		log.Info().Str("port", httpPort).Msg("unified-gateway HTTP listening")
		log.Info().Str("port", grpcPort).Msg("unified-gateway gRPC proxy listening")
	...
	<-ctx.Done()
	log.Info().Msg("shutting down unified-gateway...")
...
func osvRouter() http.Handler {
	...
	r.Get("/vulns/{id}", func(w http.ResponseWriter, r *http.Request) {
		http.Error(w, "proxied to vulnerability-service:50051", http.StatusServiceUnavailable)
	})
