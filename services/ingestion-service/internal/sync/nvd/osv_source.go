// Package osv implements a DataSource for the OSV (Open Source Vulnerabilities) database.
...
func extractCVSS(entry osvEntry) (score float64, vector string) {
	for _, s := range entry.Severity {
		if s.Type == "CVSS_V3" || s.Type == "CVSS_V3_1" {
			// Score is embedded in vector; parse if needed
			vector = s.Score
			// Quick parse: look for /CVSS:3.1/... baseScore is not in vector,
			// we just store vector and return 0 score (will be computed by consumer)
			return 0, vector
		}
	}
	return 0, ""
