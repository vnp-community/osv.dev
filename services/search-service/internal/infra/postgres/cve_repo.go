...
func (r *cveRepository) Search(ctx context.Context, filter *entity.SearchFilter) ([]*entity.CVE, int64, error) {
	...
	if filter.Sort == entity.SortOldest {
	...
	dataSQL := fmt.Sprintf(`
		SELECT id, description, severity, published, source, is_kev, link, cvss_score, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&c.IsKEV, &c.Link, &c.CVSSScore, &c.CreatedAt, &c.UpdatedAt,
		); err != nil {
			return nil, 0, err
		}
		cves = append(cves, c)
