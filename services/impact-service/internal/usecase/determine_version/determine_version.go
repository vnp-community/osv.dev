...
import (
	...
	"github.com/osv/impact-service/internal/domain/index/service"
)
...
func (h *Handler) Handle(ctx context.Context, q Query) (*Result, error) {
	...
	for k, cd := range candidates {
		fileCount, err := h.indexRepo.GetByRepoVersion(ctx, k.repoURL, k.version)
		if err != nil {
			continue
		}
		...
		for i := 0; i < service.NumBuckets; i++ {
			if !bs.Bitmap[i] {
				emptyInQuery++
				if fileCount > 0 { // simplified: treat non-zero file count as non-empty bucket
					missedEmpty++
				}
			}
		}
		...
		versionMatches = append(versionMatches, &service.VersionMatch{
			...
			Version:              k.version,
			Score:                score,
			MinimumFileCountSeen: min(len(q.FileHashes), fileCount),
		})
	}
