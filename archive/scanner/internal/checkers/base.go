...
// Build compiles all regex patterns in the definition and returns a *entity.Checker.
// Returns an error if:
//   - Name is empty
//   - VendorProduct is empty
//   - Any vendor or product is not lowercase
//   - Any regex pattern is invalid
func (def CheckerDef) Build() (*entity.Checker, error) {
	if def.Name == "" {
	...
	return entity.NewChecker(def.Name, containsRe, versionRe, filenameRe, def.VendorProduct, ignoreRe), nil
}
