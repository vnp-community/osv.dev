...
// Bucket returns a CloudStorage for the given bucket name.
func (p *GCSStorageProvider) Bucket(name string) CloudStorage {
	return NewGCSClient(p.client, name)
}

// ReadObject reads the raw contents of an object from GCS.
...
// ReadObjectAttrs reads the attributes of an object from GCS.
...
// WriteObject writes a complete byte slice to a GCS object with retries.
...
// Objects returns an iterator over GCS objects that match the prefix.
