...
// RoleAssignmentRepository manages user↔role↔scope bindings for DefectDojo RBAC.
type RoleAssignmentRepository interface {
	Create(ctx context.Context, ra *entity.RoleAssignment) error
	FindByUserID(ctx context.Context, userID uuid.UUID) ([]*entity.RoleAssignment, error)
	FindByUserAndResource(ctx context.Context, userID uuid.UUID, scope entity.RoleScope, resourceID *uuid.UUID) (*entity.RoleAssignment, error)
	Delete(ctx context.Context, id uuid.UUID) error
	ListUsersForResource(ctx context.Context, scope entity.RoleScope, resourceID uuid.UUID) ([]*entity.RoleAssignment, error)
}

// PermissionCache caches permission check results to reduce DB round-trips.
// The Redis-backed implementation in adapter/repository/redis/ provides 60s TTL.
type PermissionCache interface {
	Get(ctx context.Context, key string) (allowed bool, reason string, found bool)
	Set(ctx context.Context, key string, allowed bool, reason string)
